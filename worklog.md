...
---
Task ID: unified-redesign-2328
Agent: Super Z (main agent)
Task: Единый кровавый карнавал на ВСЕХ страницах + motion-анимации (лёгкие), ставки/кэшаут/пеймент на 2328.io, панель позиций, тесты полного цикла, проверка рефералки, удаление охватов + мотивация шеринга.

Work Log:
- ФУНДАМЕНТ: motion@13.4.4 установлен; layout.tsx — body nrld-root тёмный (#08070B, белый текст), viewport dark, landing.css импортирован ГЛОБАЛЬНО (nrld-* на всех страницах); Manrope уже был в layout.
- CSS-ШИМ (landing.css): .nrld-page перекрашивает легаси-страницы без переписывания каждого класса — bg-white/bg-white/*/пастельные градиенты → ночь, text-[#10161d] ступени → белые ступени, акценты (blue/purple/orange/green/gold) → кровь/золото, nr-collab-shimmer → белый градиент с кровью, nr-collab-hero → кровавая ночь, nr-glass/nr-glass-deep/nr-btn-glow/nr-logo → тёмный chrome; nrld-rise/nrld-rise-late/nrld-tap — лёгкие CSS-анимации (transform/opacity, reduced-motion учтён).
- СТРАНИЦЫ ПЕРЕКРАШЕНЫ: /market (+thanks), /future, /predict, /collab, /pnl, /terms, /creators (LegalShell переписан в тёмный), Menu variant="dark" везде; /pnl переписан полностью (nrld-page, THE LEDGER).
- КАШАОТ 2328.io: схема Bet + claimed/claimedAt/payoutOrderId (db push, индексы); src/lib/bet/cashout.ts — claimableSummary, cashoutWonBets (одна 2328-выплата на все won&!claimed, сеть по формату кошелька TRX-TRC20/Ethereum-ERC20, claim ТОЛЬКО после успешного инвойса, money-op логи), unclaimBetPayout; is2328WebhookConfigured в webhook.ts; POST/GET /api/me/cashout (rate 4/min, bad_wallet 400, no_claimable 409); webhook /api/webhooks/2328 — ветка bw-*: completed → лог, failed/cancelled → unclaim (деньги снова доступны).
- ПОЗИЦИИ: /api/me/bets отдаёт claimed + claimableCents/claimedTotalCents/payoutsEnabled; PositionsPanel.tsx — тёмная панель (LazyMotion domAnimation ~5kb strict, stagger-строки, spring на claimable, AnimatePresence для кэшаута), empty-стейт с CTA; на /pnl; в BetPanel после победы хинт «кэшаут — в pnl».
- ОХВАТЫ УДАЛЕНЫ: VideoCard — views-чип (pseudoViews) и импорт Eye/pseudoViews убраны; ProfilePanel — «your reach» (clicks/unique/byType) удалены, теперь «share & earn»: реферальные paid/accrued (единственные мотивирующие цифры), бейджи, invite-линк + нативный share.
- МОТИВАЦИЯ ШАРИНГА: ShareSeam.tsx — mode clip|invite (navigator.share → фолбэк копирование, ?ref= через withRef, трекает share_click); в BetPanel после резолва кнопки «шарить кадр» (/v/CODE?ref=) и «приведи глаз — 20%»; track вынесен в lib/bet/trackClient (без круговых импортов).
- ТЕСТЫ: scripts/cashout_selftest.mjs (npm run test:cashout) — 37/37 PASS против dev: crypto-ставка → инвойс мока → битая подпись 401 (БД не меняется) → webhook paid → active → пулы → резолв → пари-мьютюэль payout (540) → реф проигравшего атрибутируется (rr-… = floor(60×0.2×500/600)=10¢) → claimable → bad_wallet → кэшаут T…→ TRX-TRC20 → claimed=1/bw-* → повторный 409 → payout failed → unclaim → claimable вернулся → повторный кэшаут (новый bw-) → completed → claimed=1 → позиции claimed/claimable. bet_selftest 30/30 PASS (demo-цикл не сломан). .env песочницы: BET_WINDOW_SEC=25 + ADMIN_SECRET (без 2328-ключей → demo-режим).
- БРАУЗЕР (agent-browser, desktop+iPhone 14): /, /feed, /pnl, /market, /future, /predict, /collab, /terms — тёмный карнавал, белый текст, скриншоты в download/unified_*.png; demo-ставка через UI → позиция REAL/in the pool/$1 в /pnl; кнопки «шарить кадр»/«приведи глаз» в результат-панели; консоль чистая.
- НАЙДЕНО И ПОЧИНЕНО: Prisma-клиент устарел в памяти dev-сервера после db push («Unknown argument claimed») — потребовался полный рестарт процесса (kill дерева next dev + fresh bun run dev; мягкий touch не помог); /api/me/cashout для новичка возвращал захардкоженный payoutsEnabled:false — теперь честно из env; ShareSeam импортировал track из BetPanel после переноса → build error на /feed — переведён на trackClient; кэш CSS-чанка в браузере маскировал обновления шима (чистая сессия для верификации); rate-limit кэшаута 4/min ломал повторные прогоны самотеста (окно 60с между запусками).
- Тестовые данные БД вычищены (браузерная demo-ставка + раунды + TrackEvent).
