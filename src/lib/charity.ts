...
/**
 * useCountdown — возвращает оставшееся время до дедлайна.
...
export function useCountdown(deadlineIso: string) {
  ...
  const msLeft =
    now === null ? null : Math.max(0, new Date(deadlineIso).getTime() - now);
  return {
    ready: now !== null,
    msLeft,
...
/** 71:59:59 — часы суммарные (без суток), кампанный вид. */
