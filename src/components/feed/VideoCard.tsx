  ...
  return (
    ...
    >
      ...
      {/* ---------- основное видео: монтируется только в зоне active±1 ----------
          nonce позволяет перемонтировать <video> после ретрая, не трогая
          остальную карточку; битая ссылка заменяется fallback-сценой */}
      ...
      {/* ---------- премиальный fallback: SIGNAL LOST ---------- */}
      {showFallback && (
        <VideoFallback
          author={post.author}
          title={post.title}
